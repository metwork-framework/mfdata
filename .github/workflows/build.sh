...
#if test -f "${BUILDCACHE}/build_hash_mfdata_${BRANCH}_`cat .build_hash`"; then
...
rm -Rf html_doc
if test -d /opt/metwork-mfdata-${TARGET_DIR}/html_doc; then cp -Rf /opt/metwork-mfdata-${TARGET_DIR}/html_doc . ; fi
...
mkdir rpms
mv /opt/metwork-mfdata-${TARGET_DIR}/*.rpm rpms

#rm -f ${BUILDCACHE}/build_hash_mfdata_${BRANCH}_*
#touch ${BUILDCACHE}/build_hash_mfdata_${BRANCH}_`cat .build_hash`
#ls -l ${BUILDCACHE}
