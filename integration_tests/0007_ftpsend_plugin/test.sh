...
# Create and install plugin foobar2 sending file to localhost by ftp
...
bootstrap_plugin.py create --no-input --template=ftpsend foobar2
cd foobar2
cat config.ini | sed -e "s/arg_machine = /arg_machine = localhost/" -e "s/arg_user = /arg_user = mfdata/" -e "s/arg_passwd = /arg_passwd = mfdata/" -e "s/arg_directory = /arg_directory = .\/var\/in\/incoming\//" > config.ini.1
mv config.ini.1 config.ini
make release
ls -l
plugins.install "$(ls *.plugin)"
