...
This section applies for each Metwork modules (mfserv, mfbase, mfdata...). You configure each Metwork module in the same way.

## Introduction

Since Metwork modules (mfserv, mfbase, mfdata) is installed, a user is created for each module whose login is the module name, i.e. mfserv, mfbase, mfdata.

If it's the first time you log in as mfserv, mfbase or mfdata, you have to set a password for this login (as `root` user) before log in (e.g. `passwd mfserv` or `sudo passwd mfserv`).
...
Please make sure that your configuration file is not a symbolic link. In that particular case, you are in "production mode" (refer to the corresponding section below).
...
**To change daemons and services behaviour (like `nginx` listening port in our example), you have to restart services from a newly restarted terminal or from a `root` user through `service metwork restart` command.**
